...
def switch_to_second_screen():
    ...
    # Launch the second screen script using subprocess

    subprocess.Popen(["python", r"C:\Users\lucas\Documents\GitHub\ProjetoES_AtitusCC\build2\gui.py"])
    
    time.sleep(2)
    
    window.destroy()

# Update handle_gamepad_input() to handle "a" button press
def handle_gamepad_input():
    ...
    for event in pygame.event.get():
        if event.type == pygame.JOYHATMOTION:
            ...
            if joystick.get_button(0):  # Assuming button 0 is the "a" button
                switch_to_second_screen()
    window.after(100, handle_gamepad_input)

# Call handle_gamepad_input() to start listening for gamepad input
window.after(100, handle_gamepad_input)
highlight_selected()
